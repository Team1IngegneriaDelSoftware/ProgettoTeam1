...
namespace Improbable.Unity.Assets
{
    public class AssetBundleDownloader : IAssetLoader<AssetBundle>
    {
        ...
        private void DownloadAsync(IWWWRequest wwwRequest, string originalUrl, string redirectedUrl, Action<WWWResponse> callback)
        {
            ...
            if (headers == null)
            {
                wwwRequest.SendGetRequest(coroutineHost, redirectedUrl, response => DownloadAsyncResponse(response, wwwRequest, originalUrl, callback));
            }
            else
            {
                wwwRequest.SendPostRequest(coroutineHost, redirectedUrl, null, headers, response => DownloadAsyncResponse(response, wwwRequest, originalUrl, callback));
            }
        }
