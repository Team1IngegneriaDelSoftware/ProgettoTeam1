...
namespace Improbable.Unity.Assets
{
    class ExponentialBackoffRetryAssetLoader : MonoBehaviour, IAssetLoader<AssetBundle>
    {
        ...
        [Range(250, 10000)]
        public int StartBackoffTimeoutMilliseconds = 250;
        ...
        public void LoadAsset(string prefabName, Action<AssetBundle> onAssetLoaded, Action<Exception> onError)
        {
            ...
            Action runTask = () =>
            {
                assetLoader.LoadAsset(prefabName, onAssetLoaded, taskRunner.ProcessResult);
            };
            ...
            Action<Exception> onSuccess = (Exception e) =>
            {
                throw new InvalidOperationException("ExponentialBackoffRetryAssetLoader: TaskRunnerWithExponentialBackoff::onSuccess was called. This code is not supposed to be reachable.");
            };
