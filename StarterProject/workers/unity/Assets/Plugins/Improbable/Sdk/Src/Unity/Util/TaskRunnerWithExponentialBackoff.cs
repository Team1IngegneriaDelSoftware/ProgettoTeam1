    ...
    public class TaskRunnerWithExponentialBackoff<TResult>
    {
        ...
        public void ProcessResult(TResult response)
        {
            ...
            else
            {
                ...
                {
                    Debug.LogWarning(string.Format("Task [{0}] failed with error message: {1}. Retrying with {2} retries left.", identifier, result.ErrorMessage, remainingRetries));
                    Retry();
                }
                else
                {
                    isRunning = false;
                    Debug.LogError(string.Format("Task [{0}] failed with error message: {1}. No retries left. Aborting.", identifier, result.ErrorMessage));
